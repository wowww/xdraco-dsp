      ...
      <div class="section__inner">
        ...
        <div class="wrap-grid">
          <div class="grid-timeline">
            <!--            <mirage-notice v-if="$route.path.includes('valley-capture')" in-detail />-->
...
export default defineComponent({
  ...
  watch: {
    $route(to) {
      if (to.path.indexOf('/valley-capture') > -1) {
  ...
  mounted() {
    if (this.$route.path.indexOf('/valley-capture') > -1) {
      if (!this.$refs.refSection.classList.contains('mirage-play')) {
        this.$refs.refSection.classList.add('mirage-play');
      }
    }
  ...
  unmounted() {
    if (this.$route.path.indexOf('/valley-capture') < 0) {
      this.$refs.refSection.classList.remove('mirage-play');
    }
  },
  setup() {
