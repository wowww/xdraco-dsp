const collectible = {
  ...
  actions: {
    ...
    fetchCollectiblesTopTraded({ commit, rootState }) {
      ...
      this._vm.$dracoWebApi
        .get('/exd/lists', { params: { listType: 'topTraded', uniqueFlag: 1, languageCode: rootState.locale } })
        .then(({ data }) => {
          if (data.code === 200) {
    ...
    fetchCollectiblesRecently({ commit, state, rootState }, page = 1) {
      ...
      delete filters.sort;
      const params = { listType: 'recent', uniqueFlag: 1, languageCode: rootState.locale, page, ...filters };
