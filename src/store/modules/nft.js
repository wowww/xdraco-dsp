...
const nft = {
  ...
  state: {
    ...
    viewMode: localStorage.getItem('nft-view-mode') || 'ps',
  },
  getters: {
    nftGrade: (state) => {
  ...
  actions: {
    fetchNftItem({ commit, state, rootState }, seq) {
      ...
      this._vm.$dracoWebApi
        .get('/nft/character/summary', { params: { seq, languageCode: rootState.locale } })
        .then(({ data }) => {
          if (data.code === 200) {
    ...
    fetchNftItems({ commit, state, rootState }, page = 1) {
      commit('setLoading', true);
      const params = { listType: 'sale', ...state.itemsFilter, page, languageCode: rootState.locale };
    ...
    fetchNftTopTradedItems({ commit, state, rootState }, page = 1) {
      commit('setLoading', true);
      const params = { listType: 'topTraded', page, languageCode: rootState.locale };
    ...
    fetchNftRecommendedItems({ commit, state, rootState }, page = 1) {
      commit('setLoading', true);
      const params = { listType: 'recommended', page, languageCode: rootState.locale };
    ...
    fetchNftRecentItems({ commit, state, rootState }) {
      commit('setLoading', true);
      const params = { listType: 'recent', page: 1, languageCode: rootState.locale };
    ...
    fetchNftRecentHistoryItems({ commit, state, rootState }, page = 1) {
      commit('setLoading', true);
      const params = { listType: 'recent', page, ...state.historyFilter, languageCode: rootState.locale };
    ...
    fetchNftInventoryList({ commit, state, rootState }) {
      ...
      this._vm.$dracoWebApi
        .get('/nft/character/inven', { params: { transportID: item.character.transportID, languageCode: locale } })
        .then(({ data }) => {
          if (data.code === 200) {
            const filteredList = [];
