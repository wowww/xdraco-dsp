...
const mirage = {
  ...
  actions: {
    fetchValleyList({ commit }) {
      this._vm.$dracoWebApi.get('/mirage/meta/valley').then(({ data }) => {
    ...
    fetchAccountServerInfo({ commit, rootState }) {
      ...
      this._vm.$dracoWebApi
        ...
        .then(({ data }) => {
          if (data.code === 200) {
            const result = {};
            _.forEach(data.data, (server) => {
              _.forEach(server, (valley, key) => {
                result[key] = valley;
              });
            });
    ...
    fetchStakeInfo({ commit }, serverId) {
      ...
      this._vm.$dracoWebApi
        ...
        .then(({ data }) => {
          ...
          commit('setStakeInfo', { ..._.merge(valleyInfo, data.data) });
        })
        .catch(() => {
          commit('setStakeInfo', null);
        })
    ...
    fetchItem({ state, commit, rootState }, itemIdx) {
      ...
      this._vm.$dracoWebApi
        .get('/bossraid/item', { params: { itemidx: itemIdx, languageCode: rootState.locale } })
        .then(({ data }) => {
          if (data.code === 200) {
    ...
    fetchBossRaidMetaParty({ commit }) {
      this._vm.$dracoWebApi.get('/bossraid/meta/partybonus').then(({ data }) => {
        if (data.code === 200) {
    ...
    fetchBossRaidMetaWound({ commit }) {
      this._vm.$dracoWebApi.get('/bossraid/meta/wound').then(({ data }) => {
        if (data.code === 200) {
