...
const exd = {
  ...
  actions: {
    fetchMyItems({ commit, state, rootState }, page = 1) {
      ...
      const params = { listType: MyItemTabs[state.myItemTab].key, languageCode: rootState.locale, page };
    ...
    fetchTopTraded({ commit, rootState }) {
      ...
      this._vm.$dracoWebApi
        .get('/exd/lists', { params: { listType: 'topTraded', languageCode: rootState.locale } })
        .then(({ data }) => {
          if (data.code === 200) {
    ...
    fetchRecently({ commit, state, rootState }, page = 1) {
      ...
      delete filters.sort;
      const params = { listType: 'recent', languageCode: rootState.locale, page, ...filters };
    ...
    fetchItems({ commit, state, rootState }, page = 1) {
      commit('setLoading', true);
      const params = { listType: 'explorer', ...rootState.search.filter, page, languageCode: rootState.locale };
    ...
    fetchTradeItem({ commit, state, rootState }, tradeUID) {
      if (tradeUID) {
        commit('setLoading', true);
        this._vm.$dracoWebApi
          .get('/exd/tradeitem', { params: { tradeUID, languageCode: rootState.locale } })
          .then(({ data }) => {
            if (data.code === 200) {
    ...
    fetchMyItem({ commit, state, rootState }, itemUID) {
      if (itemUID) {
        commit('setLoading', true);
        this._vm.$dracoWebApi
          .get('/exd/item', { params: { itemUID, languageCode: rootState.locale } })
          .then(({ data }) => {
            if (data.code === 200) {
    ...
    fetchPrice({ commit }) {
      this._vm.$dracoApi.post(`/wallet/prices/draco/lastest`).then(({ data }) => {
    ...
    fetchExdMeta({ commit }) {
       ...
       * }
       */
      this._vm.$dracoWebApi.get('/exd/boxmeta').then(({ data }) => {
        const metaList = [];
