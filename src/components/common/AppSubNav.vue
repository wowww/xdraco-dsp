<template>
  <div class="subnav" :class="[subNav, $route.path === '/coin/drain' && 'light']">
    <div class="wrap-sub">
      <router-link class="logo" :to="'/' + subNav">
      ...
      <v-list id="navDsp" class="nav-dsp">
        <v-list-item-group>
          ...
          >
            <span class="text" :class="item.name.toLowerCase()">{{ t(item.name) }}</span>
          </v-list-item>
        </v-list-item-group>
      </v-list>
    </div>
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const selectItem = computed(() => {
      return items.value.findIndex((item) => item.active && item.active.findIndex((activeItem) => root.$route.path.startsWith(activeItem)) > -1);
    });
