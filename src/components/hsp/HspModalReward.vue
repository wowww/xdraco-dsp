    ...
    <v-card class="dialog-contents" tile dark>
      ...
      <v-card-text class="additional pb-0 reward">
        ...
        <div class="reward-box">
          ...
          <v-btn @click="fetchMy" width="24" height="24" fab icon :disabled="loadingMy">
            <img :class="{ on: loadingMy }" class="icon-refresh" src="@/assets/img/common/hsp-dialog/ico-refresh.webp" />
          </v-btn>
        </div>
        <p class="usd" v-if="my && price">$ {{ (Number(my.claimAmount) * Number(price.USDHydraRate)) | currency(2) }}</p>
      </v-card-text>

      <v-divider v-if="isMobile" />

      <v-card-actions class="actions">
        <button type="button" class="btn-stake btn-reward" @click="reward" :disabled="loadingMy || loading">
