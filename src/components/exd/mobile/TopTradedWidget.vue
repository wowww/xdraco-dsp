<template>
  <div class="wrap-swiper">
    <swiper ref="swiperTopTraded" class="list-item recommended" :class="{ rank: selectedTab === 0 }" :options="swiperOptions">
      <swiper-slide class="list-item__item" v-for="(item, itemIdx) in items.slice(0, 3)" :key="`top-traded-item-${item.endTime}`">
        <em class="rank-badge" v-if="selectedTab === 0">
...
export default defineComponent({
  ...
  components: { CharacterClassThumb, ExdHydraPrice, ItemCard, Swiper, SwiperSlide },
  props: {
    collectibles: {
      type: Boolean,
      default: false,
