<template>
  <div class="card-content">
    ...
    <div class="wrap-button">
      <button type="button" class="btn btn-secondary" @click="reward" :disabled="winLoading">
...
import { defaultRound, RoundState } from '@/components/dsp/round/round-utils';
import DspItemCard from '@/components/dsp/round/DspItemCard';
import { useDay, useWemixLogin } from '@/composables/round';
...
export default defineComponent({
  ...
  setup({ round }, { root, emit }) {
    ...
    const availableEndDate = useDay(round.endDate).add(1, 'year').format(t('w.rounds.date.format'));
    ...
    const showErrorModal = (errorResponse) => {
      ...
      const errorTitle = t('t.dsp.reward.failed');
      let errorMessage = '';
        ...
        .then(({ data }) => {
          ...
          root.$wemix.openQR(
            ...
            (success) => {
              ...
              setCommonModal({
                open: true,
                title: t('s.rounds.reward.complete'),
                desc: t('s.rounds.reward.complete.desc'),
