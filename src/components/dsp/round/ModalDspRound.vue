<template>
  <v-dialog content-class="dialog-stake theme-dark" v-model="open" max-width="640" persistent :fullscreen="!!md.mobile()">
      ...
      <div class="card-content">
        <div class="wrap-char">
          <div class="char-info" v-if="represent">
            ...
            <div class="amount" v-if="balance && balance">
              <dl class="balance">
          ...
          <div class="char-detail">
            <div class="wrap-detail">
              <h3 class="title" v-if="staking && not">{{ t('t.stake.popup.pre-stake') }}</h3>
              <h3 class="title" v-else-if="ongoing && not">{{ t('t.stake.popup.stake-earlier') }}</h3>
              <h3 class="title" v-else>{{ t('w.rounds.stake.more') }}</h3>
                  ...
                  <div class="odds">
                    <v-tooltip top>
                      <template v-slot:activator="{ on, attrs }">
                        <span v-bind="attrs" v-on="on" class="value">{{ currentRound.myinfo.currentOdds | currency(4) }}</span
                        >%
                      </template>
          ...
          <div class="wrap-form">
            <div class="fieldset editable" :class="{ alert: errorMessage }">
              <div>
                ...
                <input type="text" :placeholder="t('w.stake.quantity')" v-model="stakeDraco" @input="skateOnlyNum" />
              </div>
              <span class="add-draco" v-if="entryFee > 0">
                <em class="add">+</em><span class="ico-darco ico-xs add-inside">{{ entryFee }}</span>
                <em class="fee">(Entry fee)</em>
              </span>
              <div class="in-msg" v-if="!md.mobile() && prevStake > 0">
...
import { usePossibleMore, useRoundParticipate, useRoundState, useTime } from '@/composables/round';
...
export default defineComponent({
  ...
  setup(props, { root, emit }) {
    ...
    const showErrorModal = (errorResponse) => {
      ...
      const errorTitle = t('t.dsp.stake.failed');
      let errorMessage = '';
    ...
    const stake = async () => {
      ...
      const draco = root.$wemix.token('DRACO').address || dracoStorage.get(STORAGE_KEY.DRACO_WALLET);
      loading.value = true;
      root.$wemix
        .makeUnsignedTx(49, 'tornado', to, draco, 0, 'stake', [`${currentRound.value.dspID}`, `${stakeDraco.value}`])
        .then(({ data }) => {
          console.log(data);
          if (data.code !== 200) {
            showErrorModal(data);
