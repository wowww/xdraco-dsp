<template>
  <div class="dsp-preround">
    <button type="button" class="btn btn-sm btn-logout" v-if="accessToken" @click="logOut">Logout</button>
    ...
    <div class="wrap-button">
      ...
      <template v-else>
        <button type="button" v-if="validAll" class="btn btn-secondary" @click.stop="openEvenJoin()">JOIN NOW</button>
        <button type="button" v-else-if="joinable.period === 0" class="btn" disabled="disabled">
          {{ t('s.rounds.event.early.end') }}
        </button>
        <button type="button" v-else-if="joinable.nonJoin === 0" class="btn btn-disabled" @click.stop="openError(60306)">
    ...
    <v-dialog content-class="dialog-launch theme-dark" v-model="openEvent" max-width="720" persistent :fullscreen="!!md.mobile()">
      <div class="content">
        <v-btn class="btn-close" @click="closeEvent()" fab plain x-small>
        ...
        <div class="event-content">
          <template v-if="!joinComplete">
            <strong class="notice alert" v-if="!validAll">
            ...
            <template v-if="!joinComplete">
              <p class="deadline">
                <time datetime="2021-11-08T:23:59:59+08:00" v-html="t('s.rounds.event.description.2')"
                  >Event ends on November 8th 2021, 23:59:59 (UTC+8)
                </time>
              </p>
              <button type="button" v-if="!validAll" aria-disabled="true" class="btn">
                <template v-if="joinable.nonJoin === 0">{{ t('s.rounds.event.message.already') }}</template>
                <template v-else>{{ t('s.rounds.event.message.requirement') }}</template>
              </button>
              <button type="button" v-else class="btn btn-secondary" :disabled="loading" @click="postEventAirDrop">
    ...
    <v-dialog content-class="dialog-common" v-model="eventError" max-width="480" persistent>
      <div class="content">
        <v-btn class="btn-close" @click="eventError = 0" fab plain x-small>
        ...
        <div class="wrap-notice">
          <div class="msgbox vcenter">
            <div>
              ...
              <div class="desc" v-if="eventErrorCode === 60306" v-html="t('s.rounds.event.message.already')"></div>
  ...
  setup(props, { root }) {
    ...
    const wemixSdkLogin = () => {
      root.$wemix.openQR(
        ...
        (fail) => {
          console.log('Login Result: Failed please check logs in DevTool(F12)');
          console.log(fail);
        },
      );
    };
