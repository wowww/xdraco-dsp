    ...
    <div class="list-content">
      <div class="active-info">
        ...
        <div class="info">
          ...
          <div class="desc" v-if="history.type === 1001">{{ t('s.account.history.stake', [history.seasonID, history.roundID]) }}</div>
          <div class="desc" v-else-if="history.type === 1002">{{ t('s.account.history.stake-more', [history.seasonID, history.roundID]) }}</div>
          <div class="desc" v-else-if="history.type === 1003">{{ t('s.account.history.pre-stake', [history.seasonID, history.roundID]) }}</div>
          <div class="desc" v-else-if="history.type === 1004">{{ t('s.account.history.stake-more', [history.seasonID, history.roundID]) }}</div>
          <div class="desc" v-else-if="history.type === 1005">{{ t('s.account.history.round.closed', [history.seasonID, history.roundID]) }}</div>
          <div class="desc" v-if="history.type === 1006">
            {{ t('s.account.history.round.win', [history.seasonID, history.roundID, history.winner.worldName, history.winner.characterName]) }}
          </div>
          <div class="desc" v-else-if="history.type === 1007">
          ...
          ></div>
          <div class="desc" v-else-if="history.type === 2002 || history.type === 5002" v-html="t('s.exd.history.sell', [history.itemName])"></div>
          <div class="desc" v-else-if="history.type === 2003 || history.type === 5003" v-html="t('s.exd.history.cancel', [history.itemName])"></div>
          <div class="desc" v-else-if="history.type === 2004 || history.type === 5004" v-html="t('s.exd.history.listing', [history.itemName])"></div>
          <div class="desc" v-else-if="history.type === 3001" v-html="t('w.hsp.history.success-stake', [history.itemName])"></div>
          <div class="desc" v-else-if="history.type === 3002" v-html="t('w.hsp.history.success-unstake', [history.itemName])"></div>
          <div class="desc" v-else-if="history.type === 3003" v-html="t('w.hsp.history.success-reward', [history.itemName])"></div>
          <div class="desc" v-else-if="history.type === 6001" v-html="t('w.mirage.smelt.draco')"></div>
          <div class="desc" v-else-if="history.type === 6002" v-html="t('w.mirage.refine.hydra')"></div>
          <div class="desc" v-else-if="history.type === 6003" v-html="t('w.mirage.charge.vigor')"></div>
          <div class="history">
            ...
            <template v-else-if="history.type === 2001">
              - <span class="ico-darco ico-xs add-inside" v-if="history.price">{{ history.price | currency(2) }}</span>
            ...
            <template v-else-if="history.type === 5001">
              - <span class="ico-hydra ico-xs add-inside" v-if="history.price">{{ history.price | currency(2) }}</span>
              <template v-if="history.activityStatus === 0"> (Failed) </template>
              <template v-else-if="history.activityStatus === 2"> (Pending) </template>
            </template>
...
export default defineComponent({
  ...
  setup({ history }) {
    const i18n = useI18n();
    const historyDate = useDay(history.date).format(i18n.t('w.rounds.date.format'));
