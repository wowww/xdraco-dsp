<template>
  ...
  >
    <v-btn-toggle class="wrap-button-chart wrap-button-chart justify-end" :value="chartPeriod" mandatory borderless>
    ...
    <div class="indicator-currency">
      <div class="currency" data-color="red">
        <img src="@/assets/img/common/logged-in/logo-hydra.webp" height="20" alt="HYDRA" />
        <span class="name">HYDRA</span>
        <span class="line" style="background-color: rgba(153, 153, 265, 1)"></span>
      </div>
      <div class="currency" data-color="red">
        <img src="@/assets/img/common/logged-in/logo-draco.webp" height="20" alt="DRACO" />
  ...
  setup(props, { root }) {
    ...
    const generateChartData = () => {
      if (draco.value[chartPeriod.value] && hydra.value[chartPeriod.value]) {
        ...
        const hydraDatas = [];
        const dracoCurrency = props.currentCurrency === 'usd' ? 'USDDracoRate' : 'DracoPrice';
        const hydraCurrency = props.currentCurrency === 'usd' ? 'USDHydraRate' : 'HydraPrice';
        const labels = [];
        const priceData = [];
        ...
        hydraDatas.forEach((item) => {
          ...
          const price = item[hydraCurrency] ? item[hydraCurrency] : item.HydraPrice;
          const dayjsDate = dayjs(`${date} +00:00`, 'YYYY-MM-DD HH:mm:ss Z');
          const diff = Math.round(dayjsDate.diff(now) / 1000);
          if (chartPeriod.value !== 'daily' || (chartPeriod.value === 'daily' && diff >= 0)) {
            priceHydraData.push({
              x: dayjsDate.toISOString(),
              y: price,
              label: `${props.currentCurrency.toUpperCase()} / HYDRA ${Number(price).toLocaleString(undefined, {
        ...
        datas.forEach((item) => {
          const date = `${item.CreatedDT}`;
          const price = item[dracoCurrency] ? item[dracoCurrency] : item.DracoPrice;
          const dayjsDate = dayjs(`${date} +00:00`, 'YYYY-MM-DD HH:mm:ss Z');
          const diff = Math.round(dayjsDate.diff(now) / 1000);
          ...
          if (chartPeriod.value !== 'daily' || (chartPeriod.value === 'daily' && diff >= 0)) {
            priceData.push({
              x: dayjsDate.toISOString(),
              y: price,
              label: `${props.currentCurrency.toUpperCase()} / DRACO ${Number(price).toLocaleString(undefined, {
                minimumFractionDigits: 4,
                maximumFractionDigits: 4,
              })}`,
            });
        ...
        state.chartOptions = {
          ...
          scales: {
            x: {
              ...
              display: false,
              min: chartPeriod.value === 'daily' ? now.toISOString() : datas[0].CreatedDT,
    ...
    watch(
      () => [draco.value[state.selectedUsdChartPeriod], hydra.value[state.selectedUsdChartPeriod]],
      () => {
        generateChartData();
      },
    );
