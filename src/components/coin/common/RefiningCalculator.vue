<template>
  <v-dialog content-class="refining-pop" :transition="md.mobile() !== null ? 'dialog-bottom-transition' : ''" v-model="refiningPop">
    <template v-slot:activator="{ on, attrs }">
      <button type="button" v-bind="attrs" v-on="on" class="btn-calc">
        <span>{{ $t('t.hydra-refiningpop.tit') }}</span>
      </button>
    </template>

    <!-- <div class="content" :class="{ 'pop-mobile': isMobile }"> -->

    <validation-observer tag="div" class="content" v-slot="{ invalid, errors }">
      <v-btn class="btn-close" @click="refiningPop = false" fab plain x-small>close</v-btn>
      ...
      <validation-provider tag="div" rules="required|min_value:1|max_value:500" class="text-area">
        <div class="input-wrap">
          ...
          <input type="text" placeholder="1~500" v-model="stage" @input="inputOnlyNum" />
        </div>
        <!--        <a href="#" class="btn-calc">CALC</a>-->
      </validation-provider>
      <div class="info-area" :class="{ empty: invalid }" :data-empty-message="$t('t.hydra-refiningpop.empty')">
        <p class="stage-number" v-show="!invalid">{{ $t('w.hydra-refiningpop.number') }} {{ stage }}</p>
        <ul class="list" v-show="!invalid">
          <li>
