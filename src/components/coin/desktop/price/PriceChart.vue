  ...
  <section class="section">
    <div class="section__inner">
      ...
      <div class="wrap-chart">
        ...
        <p class="date" v-if="derbyDate">{{ derbyDate }}</p>
...
export default defineComponent({
  ...
  setup() {
    const { t } = useI18n();
    const { draco, derby } = useState(['draco', 'derby']);
    ...
    const derbyDate = computed(() => {
      if (derby.value) {
        ...
        return t('date.format.decoration', [dateFormatFromString(`${lastEl.CreatedDate} 00:00:00`, 'UTC+8')]);
      }
      return null;
    });
