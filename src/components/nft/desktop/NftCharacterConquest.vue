<template>
  <div ref="section">
    ...
    <div class="wrap-conquest">
      <ul class="list-conquest" v-if="building">
        <li v-for="({ key, className }, idx) in buildingKeys" :key="`conquest-${idx}`" :class="className">
          <dl class="level" v-if="building[key * 1000000]">
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const fetchApi = () => {
      state.loading = true;
      root.$dracoWebApi
        .get('/nft/character/building', { params: { transportID: item.value.character.transportID, languageCode: locale.value } })
        .then(({ data }) => {
          if (data.code === 200) {
