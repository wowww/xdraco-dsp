<template>
  <div ref="section">
    <h3 class="title-sub-detail" v-if="trainings && Object.keys(trainings).length > 0">{{ t('t.nft.training') }}</h3>
    ...
    <div class="training" v-if="!loading && trainings && Object.keys(trainings).length > 0">
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const fetchApi = () => {
      ...
      state.loading = true;
      root.$dracoWebApi
        .get('/nft/character/training', { params: { transportID: item.value.character.transportID, languageCode: locale.value } })
        .then(({ data }) => {
          if (data.code === 200) {
