<template>
  <div ref="section">
    ...
    <div v-else-if="!loading && stats" class="wrap-stat">
      <div class="wrap-stat__inner">
        <ul class="list-stat">
          ...
          <li v-for="(stat, statIdx) in stats.mainstats" :key="`stat-summary-${statIdx}`">
            <img :src="stat.iconPath" :alt="stat.statName" width="32" class="icon-subject" v-if="isMobile" />
            <span class="subject">{{ stat.statName }}</span>
            <strong class="value">{{ stat.statValue }}</strong>
          </li>
        </ul>
        <div class="wrap-score" v-if="isMobile && item">
        ...
        <div class="wrap-btn">
          <v-dialog content-class="dialog-stat-info theme-dark" v-model="dialogStatInfo" max-width="720" persistent :fullscreen="isMobile">
            <template v-slot:activator="{ on, attrs }">
              <button type="button" v-bind="attrs" v-on="on" class="btn">
      ...
      <div class="data-empty" v-if="!loadInfo" :data-empty-message="t('s.nft.trade-type.playing')"></div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const fetchApi = () => {
      console.log('fetchStat', item.value);
      state.loading = true;
      root.$dracoWebApi
        .get('/nft/character/stats', { params: { transportID: item.value.character.transportID, languageCode: locale.value } })
        .then(({ data }) => {
