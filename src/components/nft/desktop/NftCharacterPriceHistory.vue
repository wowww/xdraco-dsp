<template>
  <div ref="section">
    <h3 class="title-sub-detail" v-show="histories">{{ t('t.nft.price.history') }}</h3>
    ...
    <div v-else-if="!loading && histories" class="price-history">
      ...
      <div class="wrap-chart" v-if="chartData" v-show="false">
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const fetchApi = () => {
      ...
      state.loading = true;
      root.$dracoWebApi
        .get('/nft/character/tradehistory', { params: { tokenID: item.value.nftID, languageCode: locale.value } })
        .then(({ data }) => {
          if (data.code === 200) {
            ...
            if (state.histories.length > 0) {
              ...
              state.histories
                ...
                .reverse()
                .forEach((h, hIdx) => {
                  labels[hIdx] = useDay(h.tradeTS).locale(locale.value).format('MMM. DD');
                });
