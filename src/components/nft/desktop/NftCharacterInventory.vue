<template>
  <div ref="section">
    ...
    <div class="wrap-inventory">
      ...
      <v-tabs-items class="wrap-tabs-item" v-model="selectedTab">
        <v-tab-item class="wrap-item-square inventory" v-for="(subList, subIdx) in filteredList" :key="`inventory-sub-${subIdx}-list`">
          <div class="total">
            <em class="number" v-if="inventories">{{ subList.length || 0 }}</em> item
          </div>
          <ul class="list-item" v-if="inventories">
            <li
            ...
            >
              ...
              <span class="tier" v-if="Number(subItem.tier) > 1">{{ Number(subItem.tier) | roman }}</span>
              <span class="quantity" v-if="subItem.stack > 0">{{ subItem.stack | currency(0) }}</span>
            </li>
          </ul>
          <div class="wrap-btn" v-show="false">
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const fetchApi = () => {
      ...
      state.loading = true;
      root.$dracoWebApi
        .get('/nft/character/inven', { params: { transportID: item.value.character.transportID, languageCode: locale.value } })
        .then(({ data }) => {
          if (data.code === 200) {
    ...
    const fetchItemDetail = (itemUID, tabIdx, idx) => {
      ...
      root.$dracoWebApi
        .get('/nft/character/itemdetail', {
          params: { transportID: item.value.character.transportID, class: item.value.character.class, itemUID, languageCode: locale.value },
        })
        .then(({ data }) => {
          if (data.code === 200) {
