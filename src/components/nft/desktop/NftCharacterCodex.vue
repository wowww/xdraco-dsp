  ...
  <div ref="section">
    ...
    <div class="wrap-circle-progress codex">
      <div class="wrap-item-chart">
        <div class="chart-item" v-for="(cd, cdIdx) in codex" :key="`codex-${cdIdx}`">
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const fetchApi = () => {
      state.loading = true;
      root.$dracoWebApi
        .get('/nft/character/codex', { params: { transportID: item.value.character.transportID, languageCode: locale.value } })
        .then(({ data }) => {
          if (data.code === 200) {
            const codex = [];
