<template>
  <mirage-character :character="character" show-state :hide-score="injured" @click="openCharacterInfo">
    <div class="mirage-character__status" v-if="!hideInjured && injured" :data-character-status="character.State">
    ...
    <div class="wrap-button" v-if="!hideAction">
      ...
      <div class="mirage-character__sealed-time" v-else @click="openCharacterInfo">
        <span>Can be sealed after</span>
        <vue-countdown :time="sealRemaining" v-slot="{ days, hours, minutes, seconds }">
...
import { fromNow } from '@/filters/date';
    ...
    const openSeal = () => {
      ...
      if (closed) {
        setCommonModal({
          ...
          desc: t('s.mirage.message.lockout.seal'),
          primary: 'OK',
          onPrimary() {},
          onClose() {},
        });
