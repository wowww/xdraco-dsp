<template>
  <v-dialog
    ...
    persistent
  >
    <div class="content" v-if="accountInfo && recoverCharacter">
      ...
      <div class="critically-info">
        <div class="char" v-if="recoverCharacter">
        ...
        <dl class="critically-count">
          ...
          <dd>
            <vue-countdown class="mirage-character__cooltime" auto-start :time="restRemaining" v-slot="{ days, hours, minutes }">
...
export default defineComponent({
  name: 'ModalRecover',
  components: { MirageCharacter, MirageVigor, MirageRosterCharacterItem, MirageCharacterItem, VueCountdown },
  props: {},
  setup(props, { root }) {
    const { t } = useI18n();
    ...
    const confirmHeal = () => {
      setCommonModal({
        open: true,
        title: t('t.mirage.message.confirm.recover'),
        desc: '',
        primary: 'Recover',
