                    ...
                    <div class="input-box">
                      ...
                      <div class="btn-wrap">
                        <img
                          ...
                          alt="clear"
                        />
                        <v-btn @click="setMaxDraco" class="max" light>{{ $t('w.hsp.main.un-stake.max') }}</v-btn>
                      </div>
                    </div>
                  </div>
              ...
              <div class="item-input-wrap" :class="{ error: errors.length > 0 }">
                <div class="info-area">
                  ...
                  <ul class="info-myitem">
                    ...
                    <li>
                      {{ t('w.rounds.draco.balance') }}<span class="draco" v-if="balance">{{ balance['DRACO'] | currency(0) }}</span>
                    </li>
                  </ul>
                </div>
                <div class="field">
                    ...
                    <div class="input-box">
                      ...
                      <div class="btn-wrap">
                        <img
                          ...
                          alt="clear"
                        />
                        <v-btn @click="setMaxHydra" class="max" light>{{ $t('w.hsp.main.un-stake.max') }}</v-btn>
                      </div>
                    </div>
                  </div>
                  <button type="button" class="btn btn-orange-mirage" :disabled="errors.length > 0" @click="allowance">
...
import { sumFee } from '@/composables/mirage';
...
export default defineComponent({
  ...
  setup(_, { root, emit }) {
    ...
    const { locale, balance, derby, hydraSupply, walletAddress } = useState(['locale', 'balance', 'derby', 'hydraSupply', 'walletAddress']);
    ...
    const showErrorModal = (errorResponse) => {
      ...
      let errorTitle = t(`t.mirage.minting.${mintingSymbolIdx.value}.incomplete`);
      let errorMessage = '';
      ...
      switch (errorCode) {
        case 500:
          ...
          break;
        default:
          errorTitle = t(`t.mirage.minting.${mintingSymbolIdx.value}.incomplete`);
    ...
    const messageSign = (nonce) => {
      ...
      console.log(message);
      const req = $wemix.requestMessageSignature(
        [mintingSymbolIdx.value === 0 ? t('w.mirage.smelt.draco') : t('w.mirage.refine.hydra')],
        ['yellow'],
        message,
      );
      $wemix.openQR(
        ...
        (success) => {
          ...
          if (last.code === 200 && last.data.Result === 0) {
            setCommonModal({
              open: true,
              title: t(`t.mirage.minting.${mintingSymbolIdx.value}.complete`),
              desc: t(`s.mirage.minting.${mintingSymbolIdx.value}.complete`),
              primary: 'OK',
              onPrimary() {
                state.loading = false;
                setVisibleMinting(false);
    ...
    const makeUnsignedTx = () => {
      ...
      const req = $wemix.requestSignatureWithMemo([t(`w.mirage.minting.${mintingSymbolIdx.value}`)], ['yellow'], state.hash);
      $wemix.openQR(
        'sign',
        req,
        (success) => {
