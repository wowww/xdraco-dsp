<template>
  <v-dialog
    ...
    persistent
  >
    <div class="content" v-if="accountInfo && stakeInfo && modalSelectedValley.valleyId != '0'">
      ...
      <div class="enlist-noti" v-if="valley && Number(valley.Info.CurrentSlotCnt) >= Number(valleyMetaInfo.MaxSlotCnt)">
        <p>{{ t('s.mirage.valley.full', [t(`t.mirage.valley-type.${modalSelectedValley.valleyId}`)]) }}</p>
      </div>
      <div class="enlist-info" v-if="valley">
        <dl>
      ...
      >
        <swiper ref="swiperEnlist" :options="!isMobile ? swiperOptions : swiperMobileOptions" v-if="availableCharacters.length > 0">
        ...
        <div class="swiper-button-prev" slot="button-prev" v-show="availableCharacters.length > 0"></div>
        <div class="swiper-button-next" slot="button-next" v-show="availableCharacters.length > 0"></div>
        <button
        ...
        >
          ...
          <span class="enlist" v-if="valley && valleyMetaInfo">
...
export default defineComponent({
  ...
  setup(_, { root }) {
    ...
    const confirmStake = () => {
      ...
      setCommonModal({
        open: true,
        title: assault.value ? t('t.mirage.message.confirm.assault') : t('t.mirage.message.confirm.enlist'),
        desc: '',
        primary: assault.value ? t('w.mirage.assault') : t('w.mirage.enlist'),
