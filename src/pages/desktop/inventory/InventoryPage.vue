    ...
    <div v-if="currentTab === 0">
      ...
      <div id="wrapFilterList" class="wrap-filter">
        ...
        <div class="wrap-filter-chip">
          ...
          <list-order hide-price v-if="false" />
        </div>
      </div>
      ...
      <ul class="list-itemwrap-card" :data-empty-message="xdracoItems && xdracoItems.length > 0 ? '' : t('s.exd.sale.no-item')">
        <li v-for="(item, itemIdx) in xdracoItems" :key="`inventory-item-${itemIdx}`">
          ...
          <div class="btn-wrap">
            ...
            <router-link :to="`/inventory/${item.ItemUID}`" class="btn btn-primary-exd">SELL</router-link>
          </div>
        </li>
      </ul>
...
export default defineComponent({
  name: 'InventoryPage',
  components: { ModalSend, ItemCard, ModalImport, MyItem, ItemInfo, ListOrder, ListFilter, ListUniqueFilter },
  setup(_, { root }) {
    ...
    const fetchItemList = (type = 'inventory') => {
      ...
      const params = { listType: type, languageCode: locale.value };
