  ...
  <section class="section">
    ...
    <div class="test2">Test2 Test2 Test2</div> -->
        ...
        <div v-if="currentTab === 0">
          ...
          <div id="wrapDateOrder" class="wrap-date-order" v-if="false">
            <list-order />
          </div>
          ...
          <div class="d-flex justify-center align-center py-10" v-if="loading">
            <v-progress-circular class="ma-5" :size="32" :width="4" color="#fff" indeterminate />
          </div>

          <div class="d-flex justify-center align-center py-16" v-if="!loading && (!xdracoItems || xdracoItems.length === 0)">
          ...
          <ul class="list-item">
            <li v-for="(item, itemIdx) in xdracoItems" :key="`inventory-item-${itemIdx}`">
              ...
              <div class="btn-wrap">
                ...
                <router-link tag="button" :to="`/exd/my-items/${item.ItemUID}`" class="btn btn-sm btn-primary-exd">SELL</router-link>
              </div>
            </li>
          </ul>

          <v-dialog content-class="dialog-card" v-model="open">
            <v-card v-if="item">
              <item-card :item="item" :ref="card" use-info :collectible="!!item.uniqueNo" />
              <button type="button" class="btn-close-dialog" @click="closeCard">Close Dialog</button>
            </v-card>
          </v-dialog>

          <div class="wrap-button" v-if="xdracoMore">
        ...
        <div v-else-if="currentTab === 1">
          <div class="not-login" v-if="exist === 0">
          ...
          <template v-else>
            ...
            <div class="d-flex justify-center align-center py-16" v-if="!loading && (!mir4Items || mir4Items.length === 0)">
            ...
            <div class="wrap-button" v-if="mir4More">
        ...
        <div v-else>
          <div class="d-flex justify-center align-center py-10" v-if="exdLoading">
            <v-progress-circular class="ma-5" :size="32" :width="4" color="#fff" indeterminate />
          </div>
          <template v-else>
            <div class="list-bar">
            ...
            <div class="d-flex justify-center align-center py-16" v-if="!loading && (!myItems || myItems.length === 0)">
...
export default defineComponent({
  name: 'InventoryPage',
  components: { ModalSend, ItemCard, ModalImport, MyItem, ItemInfo, ListOrder, ListFilter, ListUniqueFilter, Swiper, SwiperSlide },
  setup(_, { root }) {
    ...
    const fetchItemList = (type = 'inventory') => {
      ...
      root.$nextTick(() => {
        $dracoWebApi
          .get('/warehouse/lists', { params: { listType: type, languageCode: locale.value } })
          .then(({ data }) => {
            console.log(data);
    ...
    const fetchItem = (itemUID) => {
      root.$dracoWebApi
        .get('/exd/item', {
          params: { itemUID, languageCode: locale.value },
        })
        .then(({ data }) => {
          if (data.code === 200) {
