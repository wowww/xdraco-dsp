      ...
      <div class="section__inner">
        <div class="exchange-list for-list">
          ...
          <div class="list-bar">
            ...
            <v-tabs class="list-bar__tab" background-color="transparent" v-model="itemsExplorer" v-show="false">
              <v-tab>All</v-tab>
          ...
          <ul class="list-item" :data-empty-message="items && items.length > 0 ? '' : t('s.exd.sale.no-item')" style="margin-top: 24px">
            <exchange-item use-link v-for="item in items" :key="`exchange-item-${item.tradeUID}`" :exchange-item="item" card-size="sm" collectible />
          </ul>
...
import CollectibleBanner from '@/components/exd/common/CollectibleBanner';
