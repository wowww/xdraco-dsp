    ...
    <section class="section">
      <div class="section__inner">
        ...
        <div class="myitem-list">
          <v-tabs :value="selectedTab" @change="setSelectedTab" class="tab-rounds" :hide-slider="true">
            <v-tab>
        ...
        <div class="exd-ranking-list" v-if="selectedTab === 0" style="margin-top: 24px">
          <top-traded-item
          ...
          <ul class="list" v-if="topTradedItems.length === 0" :data-empty-message="t('s.exd.sale.no-item')"></ul>

          <div class="wrap-button" v-if="topTradedPage < 10">
            <button type="button" class="btn-viewmore" @click="setTopTradedPage(topTradedPage + 1)">VIEW MORE</button>
          </div>
        </div>
        <div class="myitem-list for-list" v-else>
          <div class="tab-soldout">
            ...
            <div class="wrap-filter-chip">
              ...
              <div class="btn-filter-reset" v-if="hasFilter" @click="resetAllFilter(0)">
                <span>Reset</span>
            ...
            <ul class="list-myitem">
              <li v-for="(item, itemIdx) in recentlyItems" :key="`exd-history-recently-item${item.endTime}`" @click="showItemCard(itemIdx)">
                <div class="myitem" data-sale-type="soldout">
                  <item-info :item="item.item" />
                  <div class="myitem-detail">
                    <exd-hydra-price :price="item.price" icon-size="xs" primary-class="price" :hide-usd="true" />
                    <ul class="spec">
                      <li class="seller">
            ...
            <div class="wrap-button" v-if="recentlyMore">
            ...
            <v-dialog content-class="dialog-card" v-model="openCard">
              <v-card v-if="recentlyItems.length > 0 && selectedItemIdx > -1">
                <item-card :item="recentlyItems[selectedItemIdx].item" ref="card" :collectible="!!recentlyItems[selectedItemIdx].item.uniqueNo" />
...
import { defineComponent, onMounted, reactive, toRefs, computed } from '@vue/composition-api/dist/vue-composition-api';
...
export default defineComponent({
  ...
  components: { ExdHydraPrice, ItemCard, ItemInfo, TopTradedItem, Footer, AccountInfo, ListFilter, ListUniqueFilter, Swiper, SwiperSlide },
  ...
  setup(props, { root }) {
    ...
    const { selectedTab, topTradedLoading, topTradedItems, topTradedPage, recentlyItems, recentlyMore, recentlyPage } = useExdState([
      'selectedTab',
      'topTradedLoading',
      'topTradedItems',
      'topTradedPage',
