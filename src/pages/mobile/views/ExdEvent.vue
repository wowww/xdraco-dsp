<template>
  <main class="contents exd-event">
      ...
      <div class="section__inner">
        ...
        <div class="wrap-preparate">
          ...
          <p class="desc" v-html="t('s.exd.event.prepare.desc')">
            <em>1 bonus DRACO</em> reward will be given to the first <em>50,000</em> people that register for the EXD Kickoff Event
          </p>
        </div>
            ...
            <li class="step3" :class="{ alert: !loading && check && joinParticipant >= 50000, active: !loading && check && joinParticipant < 50000 }">
              <div class="wrap-detail">
                ...
                <template v-else>
                  <span class="ico-checked"><em class="hidden">Checked</em></span>
                  <div class="goal">
                    <div class="membercount nosync" v-if="!check">
                      <small>{{ t('w.rounds.reward.total') }}</small>
                      <strong>50K</strong>
                    </div>
                    <div class="membercount" v-else>
    ...
    <v-dialog content-class="dialog-exdevent theme-dark" v-model="open" max-width="720" persistent :fullscreen="isMobile">
      <div class="content">
        <v-btn class="btn-close" @click="closeModal" fab plain x-small>
